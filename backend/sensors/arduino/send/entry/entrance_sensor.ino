...
// Sensor thresholds
constexpr float MAX_DISTANCE_CM = 15.0f;                              // HC-SR04 max
...
// Timing and buffer
const unsigned long MIN_TIME_DIFF_US = 500000UL;       // min separation between valid records in microseconds (1 s)
...
struct Record {
  ...
  float time_s;  // time in seconds with fractional microseconds
};
...
public:
  ...
  bool add(const Record& r) {
    ...
    // Check time difference vs last stored record
    int lastIdx = (head + BUFFER_CAPACITY - 1) % BUFFER_CAPACITY;
    const Record& last = buffer[lastIdx];
    // compare in microseconds to avoid float precision issues
    unsigned long last_us = (unsigned long)(last.time_s * 1000000.0f);
    unsigned long cur_us = (unsigned long)(r.time_s * 1000000.0f);

    unsigned long diff_us = (cur_us >= last_us) ? (cur_us - last_us) : (last_us - cur_us);
    if (diff_us < MIN_TIME_DIFF_US) {
      // too close -> reject
      return false;
    }
...
// ---------- time window config ----------
const int START_HOUR = 0;    // active start (08:00)
const int END_HOUR   = 23;   // active end   (22:00)
const uint64_t uS_TO_S_FACTOR = 1000000ULL;



time_t nowTime() { return time(nullptr); }
...
void setup() {
  ...
  // Connect to WiFi briefly to sync time (only if you need accurate wall-clock)
  WiFi.begin(WIFI_SSID, WIFI_PASSWORD);
  unsigned long tstart = millis();
  while (WiFi.status() != WL_CONNECTED && millis() - tstart < 7000) delay(100);
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("WiFi not available for time sync");
  }

  time_t now = nowTime();
  if (now < 1609459200UL) {
    // no reliable time -> sleep a short while and retry later
  ...
  // If currently outside active window -> sleep until next start and DO NOT run loop()
  if (!(hour >= START_HOUR && hour < END_HOUR)) {
    ...
    esp_deep_sleep_start(); // device will reset at START_HOUR and run setup() again
    // never returns
  }

  // If we reach here, we are inside the active window. Let loop() run repeatedly.
  activeWindow = true;
  // initialize lastUploadAttempt or other runtime vars if needed
...
void loop() {
  ...
  unsigned long timeout_us = (unsigned long)(MAX_DISTANCE_CM * 58.0f) + 2000UL;  // add margin
